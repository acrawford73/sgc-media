...
async def watch_recursive(path: Path, mask: Mask) -> AsyncGenerator[Event, None]:
    with Inotify() as inotify:
        for directory in get_directories_recursive(path):
            ...
            inotify.add_watch(directory, mask | Mask.MOVED_FROM | Mask.MOVED_TO | Mask.CREATE | Mask.DELETE | Mask.DELETE_SELF | Mask.IGNORED)
...
async def main():
    async for event in watch_recursive(Path('/home/ubuntu/Development/sgc-media/media_assets'), Mask.CREATE):
